...
def computeBoundaryBox(ccimg, ccsizes, image_height, image_width):
    ...
    # Iterate over the connected components and compute boundary boxes
    barcode_boxes = []
    for i in range(len(sorted_ccsizes)):
        cc_label, cc_size = sorted_ccsizes[i]
        
        # Ignore small components (adjust the threshold as needed)
        if cc_size < 100:
            break
        
        # Find the minimum and maximum coordinates for the component
        min_x = image_width
        min_y = image_height
        max_x = 0
        max_y = 0
        for y in range(image_height):
            for x in range(image_width):
                if ccimg[y][x] == cc_label:
                    min_x = min(min_x, x)
                    min_y = min(min_y, y)
                    max_x = max(max_x, x)
                    max_y = max(max_y, y)

        # Add the boundary box to the list
        aspect_ratio = (max_x - min_x) / (max_y - min_y)
        aspect_ratio2 =(max_y - min_y) / (max_x - min_x)
        if (aspect_ratio or aspect_ratio2) and (max_y - min_y > 40 or max_x - min_x > 40):
            barcode_boxes.append((min_x, min_y, max_x - min_x, max_y - min_y))
        
        # Set the pixels of the detected barcode to 0 to ignore it in the next iteration
...
def computeGaussianAveraging5x5RepeatBorder(pixel_array, image_width, image_height):
    result = createInitializedGreyscalePixelArray(image_width, image_height)
    kernel_value = [
        [1, 4, 7, 4, 1],
        [4, 16, 26, 16, 4],
    ...
    for i in range(image_height):
        for j in range(image_width):
            ...
            for a in range(-2, 3):
                for b in range(-2, 3):
                    k = min(max(i + a, 0), image_height - 1)
                    l = min(max(j + b, 0), image_width - 1)
                    count += pixel_array[k][l] * kernel_value[2 + a][2 + b]
            result[i][j] = count / kernel_sum
            
    return result

# Add a new function to detect multiple barcodes
def detectMultipleBarcodes(image_filename):
    # Read the image and convert it to grayscale
    image_width, image_height, pixel_array_r, pixel_array_g, pixel_array_b = readRGBImageToSeparatePixelArrays(image_filename)
    greyscale_pixel_array = computeRGBToGreyscale(pixel_array_r, pixel_array_g, pixel_array_b, image_width, image_height)

    # Apply image processing techniques to enhance barcode visibility
    processed_image = computeVerticalEdgesSobelAbsolute(greyscale_pixel_array, image_width, image_height)
    processed_image = computeThresholdGE(processed_image, 100, image_width, image_height)
    processed_image = computeErosion8Nbh5x5FlatSE(processed_image, image_width, image_height)

    # Perform connected component labeling to identify individual barcodes
    ccimg, ccsizes = computeConnectedComponentLabeling(processed_image, image_width, image_height)

    # Iterate over the connected components and compute boundary boxes
    barcode_boxes = []
    for i in range(1, len(ccsizes) + 1):
        start_x, start_y, width, height = computeBoundaryBox(ccimg, ccsizes, image_height, image_width)
        
        # Filter out smaller rectangles and those with aspect ratio greater than 1.8
        aspect_ratio = max(width, height) / min(width, height)
        if aspect_ratio <= 1.8:
            barcode_boxes.append((start_x, start_y, width, height))
    ...
    ax = pyplot.gca()
    for barcode_box in barcode_boxes:
        start_x, start_y, width, height = barcode_box
        rect = Rectangle((start_x, start_y), width, height, fill=False, color="red")
        ax.add_patch(rect)
    pyplot.show()
...
def main():
    ...
    for i in range(1):
        blurred_image = computeBoxAveraging5x5(edge_magnitudes, image_width, image_height) 
        filtered_image = blurred_image
    for i in range(3):    
        gaussian_filtered_array = computeGaussianAveraging5x5RepeatBorder(filtered_image, image_width, image_height)
        filtered_image2 = gaussian_filtered_array
        
    repeat_contrast_stretch = scaleTo0And255AndQuantize(filtered_image2, image_width, image_height)
    axs1[0, 1].set_title('blurred_image of image')
    axs1[0, 1].imshow(repeat_contrast_stretch, cmap='gray')
    print("blurring done")
     
    # STEP 4: Threshold the image:
    convert_to_binary = computeThresholdGE(repeat_contrast_stretch, 94, image_width, image_height)
    
    # STEP 5: Erosion and dilation:
    morphological_close = computeDilation8Nbh5x5FlatSE(convert_to_binary, image_width, image_height)
    for i in range(9): 
        if i < 2:
            morphological_close = computeDilation8Nbh5x5FlatSE(morphological_close, image_width, image_height)
        else:
            morphological_close = computeErosion8Nbh5x5FlatSE(morphological_close, image_width, image_height)
    ...
    # Usage:
    barcode_boxes = computeBoundaryBox(ccimg, ccsizes, image_height, image_width)
    ...
    # # Call the detectMultipleBarcodes function with the input image filename
    # detectMultipleBarcodes("input_image.png")
